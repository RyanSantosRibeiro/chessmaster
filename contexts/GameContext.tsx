...
export type ChessVsBotContextType = ReturnType<typeof useMultiplayer>;

const ChessMultiplayerContext = createContext<ChessVsBotContextType | undefined>(undefined);

export const ChessVsBotProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  const value = useMultiplayer();
  return <ChessMultiplayerContext.Provider value={value}>{children}</ChessMultiplayerContext.Provider>;
};

export const useChessMultiplayer = () => {
  const context = useContext(ChessMultiplayerContext);
  if (!context) {
